...
\shtsetup{
  ...
  secret-level = {},
  title = {四旋翼无人机的受限空间飞行控制与规划},
  title* = {Flight Control and Planning of Quadrotor UAV in Constrained Space},
  ...
  supervisor = {汪阳~助理教授},
  supervisor* = {Professor~Wang~Yang},
...
\chapter{基础知识} % 第二章
四旋翼动力学、运动学。坐标系选择，momentum theory，actuator disk theory，blade element theory，等等。
...
\begin{figure}[htb]
  \centering
  \IfFileExists{image/shanghaitech-emblem.pdf}{%
    \includegraphics[width=0.5\columnwidth]{image/shanghaitech-emblem.pdf}%
  }{%
    \fbox{%
      \begin{minipage}[b][2.5cm][c]{0.75\columnwidth}%
        \centering\zihao{-5}\bfseries\sffamily\color{ShtRed}%
        校徽文件 \texttt{image/shanghaitech-emblem.pdf} 缺失%
      \end{minipage}%
    }%
  }%
...
\chapter{临近效应和空气动力学} % 第三章

Proximity effect is a phenomenon that occurs when two superconductors are separated by a thin insulating barrier. The proximity effect is a direct consequence of the wave-like properties of the superconducting condensate. The proximity effect is a phenomenon that occurs when two superconductors are separated by a thin insulating barrier. The proximity effect is a direct consequence of the wave-like properties of the superconducting condensate.

\chapter{基于xxx的控制器} % 第四章

\chapter{基于xxx的trajectory planner} % 第五章

\chapter{系统整合与实验验证} % 第六章
